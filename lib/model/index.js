...
function Model (persist){
  ...
  for(var i =1;i<=this.size;i++){
    for (var j=1;j<=this.size;j++){
      this.db.insert(new TableCell(i,j,"foret"));
    }
  }
...
Model.prototype.updateRessources = function(tile){
  var self = this;
  Object.keys(tile.consommation).every(function(key){
    self.ressources[key].stock -= cost[key];
  });
  Object.keys(tile.production).every(function(key){
    self.ressources[key].stock += cost[key];
  });
}
