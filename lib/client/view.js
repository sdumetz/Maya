...
View.prototype.drawMap = function(){
  var self = this;
  src = "img/fond.png"
  for(i=2;i<=this.grid_size;i++){
    self.createBackgroundBmp(this.grid_size+2, i, src);
    self.createBackgroundBmp(i+1,  this.grid_size+1, src);
  }
  self.createBackgroundBmp(this.grid_size+2,  this.grid_size+1, src);
  ...
  for (i = 1;i <= this.grid_size;i++){
    for (j = 1;j <= i;j++){
      ...
      this.createBmp(L,C,src);
    }
  }
  for (i=1;i<=this.grid_size-1;i++){
    for (j=1;j<=this.grid_size-i;j++){
      C = this.grid_size-j+1;
      L = i+j;
      src = this.model.getImageByCoordinates(C,L);
      this.createBmp(L,C,src);
    }
...
View.prototype.createBmp = function (L, C, src){
...
View.prototype.createBackgroundBmp = function (L, C, src){
  var tile_size = this.tile_size;
