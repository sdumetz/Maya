...
function View (model,drawZone){
  ...
  this.model = model;
  this.tile_size = 100;
  this.grid_size = Math.floor(Math.min(this.docwidth,this.docheight)/(this.tile_size))*2;
...
View.prototype.draw= function(id,src){
  var oldImage = this.stage.children[id];
  var newImage = new createjs.Bitmap(src);
  newImage.x = oldImage.x;
  newImage.y = oldImage.y;
  newImage.regX =100;
  newImage.regY = 150;
  this.stage.addChildAt(newImage,id);
  this.stage.removeChildAt(id+1);
}

View.prototype.drawMap = function(diffx,diffy){
  var self = this
    , list = []
    , removeList = false
    , start = Utils.Grid.getMapCoordinates(this.x0,this.y0,{offset:self.lastTransform});
  console.log("in center of map : ",start);
  this.stage.removeAllChildren();
  var countBefore = this.stage.children.length;
  console.log("drawing from x : ",start.x-this.grid_size/2," to ",start.x + this.grid_size/2);
  console.log("drawing from y : ",start.y-this.grid_size/2," to ",start.y + this.grid_size/2);
...
View.prototype.getMapCoordinates = function(pageX,pageY){
  var x = pageX
    , y = pageY
    , angle = Math.atan(1/2)
    , xt = x/Math.cos(angle) + y/Math.sin(angle)
    , yt = -x/Math.cos(angle) + y/Math.sin(angle)
    , ratio = this.tile_size*Math.sqrt(5)
    , numX = Math.floor(xt/ratio)
    , numY = Math.floor(yt/ratio);
  return {x:numX,y:numY};
}
View.prototype.getPageCoordinates = function(mapX,mapY){
    var angle = Math.atan(1/2)
    , ratio = this.tile_size*Math.sqrt(5)/2
    , x = mapX*ratio
    , y = mapY*ratio
    , xt = x*Math.cos(angle) - y*Math.cos(angle)
    , yt = x*Math.sin(angle) + y*Math.sin(angle)
    , numX = Math.floor(xt)
    , numY = Math.floor(yt);
  return {x:numX,y:numY};
}
