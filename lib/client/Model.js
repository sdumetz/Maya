...
Model.prototype.getIn = function(start,size){
  start = start || Utils.Grid.getMapCoordinates(this.origin.x,this.origin.y,{offset:this.offset,origin:this.origin});
  size = size || this.grid_size;
  var list = new Array();
  console.log("offset : ",this.offset,"origin", this.origin);
  console.log("getting from : ",start.x-size/2, "to",start.x + size/2,"center : ",start );
...
Model.prototype.create = function(tile){
  var co = Utils.Grid.getPageCoordinates(tile.x,tile.y,{offset:this.offset,origin:this.origin});
